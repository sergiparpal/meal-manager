...
- **`dish.py`** — `Dish` dataclass: recipe model with `ingredients` dict mapping name → `bool` (True = essential, False = optional). `can_cook_with()` checks if all essential ingredients are available. Serialization uses Spanish keys (`nombre`, `tiempo_prep`, `ingredientes`) to match the JSON format.
...
- **`history.py`** — Cooking history persistence (`data/historial.json`). Maps dish name → last-cooked ISO date. Keys are normalized to lowercase on load.
- **`storage.py`** — Recipe catalog persistence (`data/platos.json`). `load_dishes()` / `save_dishes()` handle the `{"platos": [...]}` wrapper.
- **`fridge.py`** — Fridge persistence (`data/nevera.json`). Simple list of lowercase ingredient strings, deduplicated on load.
...
- `platos.json` — Recipe catalog. Wraps dishes in `{"platos": [...]}`; each dish has `nombre`, `tiempo_prep`, and `ingredientes` (name → bool).
- `nevera.json` — Fridge inventory (flat array of ingredient strings).
- `historial.json` — Cooking history (dish name → ISO date string).
